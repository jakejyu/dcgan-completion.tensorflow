...
class DCGAN(object):
    ...
    def build_model(self):
       ...
       # LSGAN
        ...
        # Completion.
        # position importance weights for center hole
        center = (self.image_size - 1.0) /2
        #position_weights = np.ones([self.image_size, self.image_size, 3])
        position_weights_center = np.ones(self.image_shape, dtype=np.float32)
        for i in range (0, self.image_size):
          for j in range (0, self.image_size):
              ...
              position_weights_center[i, j, :] = 1 / math.log2(math.sqrt((i-center)*(i-center) + (j-center)*(j-center)))


        #position importance weights for right/left part missing
        position_weights_rl = np.ones(self.image_shape, dtype=np.float32)
        for i in range (0, self.image_size):
          for j in range (0, self.image_size):
              #position_weights[i, j, :] = 1/(math.sqrt(math.sqrt(math.sqrt(j-center))))
              position_weights_rl[i, j, :] = 1 / math.log2(math.fabs(j-center))

        #position weights is 1 for random mask, i.e. don't multiply position weights with abs below

        #if config.maskType == 'center':
        #    position_weights = position_weights_center
        #elif config.maskType == 'right' or config.maskType == 'left':
        #    position_weights = position_weights_rl
        #elif config.maskType == 'random':
        #    position_weights = np.ones(self.image_shape, dtype=np.float32)

        self.mask = tf.placeholder(tf.float32, [None] + self.image_shape, name='mask')
        self.contextual_loss = tf.reduce_sum(
            tf.contrib.layers.flatten(
                #tf.abs(tf.multiply(self.mask, self.G) - tf.multiply(self.mask, self.images))), 1) # 0
                tf.multiply(position_weights_center, tf.abs(tf.multiply(self.mask, self.G) - tf.multiply(self.mask, self.images)))), 1) # 0
